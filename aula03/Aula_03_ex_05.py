...
#teste
image_grad_x = np.uint8( np.absolute(abs_grad_x) )
image_grad_y = np.uint8( np.absolute(abs_grad_y) )
cv2.imshow( "64 bits",imageSobel3x3_X )
cv2.imshow( "abs grad x", image_grad_x )
cv2.imshow( "abs grad y", image_grad_y )
